...
`--rebase kth` shifts the k-th score to -10 instead of the maximum to +10.
"""
import argparse
...
print(f"positions {n}, k {k}, rebase {args.rebase}")
print(f"KL(b||a): mean {kls.mean():.4f} p50 {np.median(kls):.4f} p95 {np.quantile(kls, .95):.4f} max {kls.max():.3f} at {int(kls.argmax())}")
